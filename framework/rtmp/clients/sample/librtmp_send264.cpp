...
_rtmp_::RtmpSession* GetSessionInstance(void)
{
	static _rtmp_::RtmpSession s_Session;
	static _rtmp_::RtmpSession::self_sptr sptr = _rtmp_::RtmpSession::Alloc();
	return sptr.get();
}
...
{  
	...
	GetSessionInstance()->Init(_rtmp_::RtmpSession::HFNEvent());
	return GetSessionInstance()->Connect(url);
}  
 ...
 */
int SendPacket(int stream_id, unsigned int nPacketType,unsigned char *data,unsigned int size,unsigned int nTimestamp)
{  
	RTMPPacket* packet;
	/*分配包内存和初始化,len为包体长度*/
	...
	/*发送*/
	int nRet =0;
	if (GetSessionInstance()->IsOpen())
	{
		nRet = GetSessionInstance()->SendPacket(packet,TRUE); /*TRUE为放进发送队列,FALSE是不放进发送队列,直接发送*/
	}
	/*释放内存*/
	free(packet);
	return nRet;  
 ...
 * @成功则返回 1 , 失败则返回0
 */
int SendVideoSpsPps(int stream_id, unsigned char *pps,int pps_len,unsigned char * sps,int sps_len)
{
	...
	/*调用发送接口*/
	int nRet = GetSessionInstance()->SendPacket(packet,TRUE);
	free(packet);    //释放内存
	return nRet;
}
...
{  
	...
	if(bIsKeyFrame){  
		...
		SendVideoSpsPps(GetSessionInstance()->Get()->m_stream_id, metaData.Pps,metaData.nPpsLen,metaData.Sps,metaData.nSpsLen);
	}else{  
	...
	int bRet = SendPacket(GetSessionInstance()->Get()->m_stream_id, RTMP_PACKET_TYPE_VIDEO,body,i+size,nTimeStamp);
