...
#include <libProto_Sip_V20/Sip_Request.h>
...
namespace	_sip_v20_ {
	...
	int Sip_Request_Register::Init(const UriDetails_INFO& Src, 
	...
	_string_big_t Sip_Request_Register::Generate( void )
	{
		...
		Str += _Dest.Coding + "@" + _Dest.Host.Host;
		if (_Dest.Host.Port != 0)
		{
			Str += ":";
			Str += _string_t::NumberToStr(_Dest.Host.Port).c_str();
		}
		...
		sptr->Content = "<sip:" + _Src.Coding + "@" + _Src.Host.Host;
		if(_Src.Host.Port != 0)
			sptr->Content += ":" + _string_t::NumberToStr(_Src.Host.Port);
	...
	int Sip_Request_Invite::Init(const UriDetails_INFO& Src,
		const UriDetails_INFO& Dest, long CSeq, bool isUdp)
	{
		_isUdp = isUdp;
		_Common.SetCSeq(CSeq, GetMethodName());
		_Dest = Dest;
	...
	int Sip_Request_Ack::Init(const UriDetails_INFO& Src,
		const UriDetails_INFO& Dest, long CSeq, bool isUdp)
	{
		_isUdp = isUdp;
		_Common.SetCSeq(CSeq, GetMethodName());
	...
	int Sip_Request_Bye::Init(const UriDetails_INFO& Src,
		const UriDetails_INFO& Dest, long CSeq, bool isUdp)
	{
		_isUdp = isUdp;
		_Common.SetCSeq(CSeq, GetMethodName());
	...
	int Sip_Request_Message::Init(const UriDetails_INFO& Src,
		const UriDetails_INFO& Dest, long CSeq, bool isUdp)
	{
		_isUdp = isUdp;
		_Common.Release();
	...
	int Sip_Request_Options::Init(const UriDetails_INFO& Src,
		const UriDetails_INFO& Dest, long CSeq, bool isUdp)
	{
		_isUdp = isUdp;
		_Common.SetCSeq(CSeq, GetMethodName());
	...
	int Sip_Request_Cancel::Init(const UriDetails_INFO& Src,
		const UriDetails_INFO& Dest, long CSeq, bool isUdp)
	{
		_isUdp = isUdp;
		_Common.SetCSeq(CSeq, GetMethodName());
