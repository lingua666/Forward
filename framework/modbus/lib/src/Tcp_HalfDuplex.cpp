...
namespace _modbus_ {
	...
	namespace _tcp_ {
		...
		THandle Tcp_HalfDuplex::Open(const _mod_string& sIP, UInt16 uPort, const HFNDestroy& hDestroy)
		{
			_Handle = NULL;
...
gt_start:
			...
			{
				if (Timestamp_type::getcurrent_microseconds() - _uTuration >= TCP_RECV_TURATION_DEF)
				{//占用总线5秒  报错
			...
			Int16 ReSendTimer, Int16 ReSendInterval)
		{
			...
			pData->ReSendTimer = ReSendTimer == -1 ? TCP_DATA_RESEND_DEF : ReSendTimer;
			pData->ReSendInterval = ReSendInterval == -1 ? TCP_RECV_INTERVAL_DEF : ReSendInterval;
