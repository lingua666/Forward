...
struct snd_ctl_elem_value {
	...
	unsigned int write_force: 1;	/* no Read-Write extend */
#define SND_CTL_ELEM_WRITE_FORCE_BIT (1<<1) /* write_force:1 */
	union {
		union {
			long value[128];
...
struct snd_ctl_elem_chain {
	unsigned short num_elems;
	unsigned short data_size; /* real byte-size to copy */
	unsigned int flags; /* SND_CTL_ELEM_WRITE_FORCE_BIT and others */
	long elem_area[128];
	/* elem_area[] should be filled in format:
	 * snd_ctl_elem_value: id.numid
	 * used data-size of value.integer.value[]
	 * snd_ctl_elem_value: value.integer.value[]
	 * ...
	 * used data-size of value.integer.value[]
	 * snd_ctl_elem_value: id.numid
	 * snd_ctl_elem_value: value.integer.value[]
...
#define SNDRV_CTL_IOCTL_ELEM_WRITE_CHAIN	_IOWR('U', 0x1d, struct snd_ctl_elem_chain)
