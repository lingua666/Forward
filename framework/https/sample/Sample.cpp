...
#include <libFrameWork_Https/Https.h>
#include <iostream>
...
FILE* g_fp = fopen("D:\\test.mp3", "wb");

void HandleData(_https_::Https<_app_string>::StatusCode Code, const char* c_szData, UInt32 uSize)
{
	switch (Code)
	{
	case _https_::Https<_app_string>::EN_Status_Data_Start:
	{
		
	}
	break;
	case _https_::Https<_app_string>::EN_Status_Data:
	{
		/*if (c_szData != NULL)
		{
			_app_string s(c_szData, uSize);
			printf("HandleData(%d) Data:%s\r\n", s.size(), s.c_str());
		}

		fwrite(c_szData, 1, uSize, g_fp);*/
	}
	break;
	case _https_::Https<_app_string>::EN_Status_Data_End:
	{
		//fclose(g_fp);
	}
	break;
	case _https_::Https<_app_string>::EN_Status_Err_Connect:
	{

	}
	break;
	case _https_::Https<_app_string>::EN_Status_Err_Encode:
	{

	}
	break;
	case _https_::Https<_app_string>::EN_Status_Err_Decode:
	{

	}
	break;
	case _https_::Https<_app_string>::EN_Status_Err_Send:
	{
...
void LibHttpTest(void)
{
	/*while (true)
	{
		X509* pCert = _https_::Ssl::LoadFileToCert("D:\\http是证书.cer");
		if (pCert != NULL)
		{
			//printf("subject name: %s\n", _https_::Ssl::GetSubjectName(pCert).c_str());
			//printf("issuer name: %s\n", _https_::Ssl::GetIssuerName(pCert).c_str());
			printf("serialNumber: %s\n", _https_::Ssl::GetCertSerialNumber(pCert).c_str());

			//_https_::Ssl::IsCertificateExpired(pCert);
		}
	...
	_https_::Https<_app_string>::HttpsClientsEnvironmentInit();
	
	while (true)
	{
		{
			_https_::Https<_app_string>::self_sptr	Http = _https_::Https<_app_string>::Alloc();
			Http->Init(false, function20_bind_Ex(HandleData));
			//Http->Post("https://nankgd.com/nkytms/getPubKey", _app_string());
			//Http->Get("https://nankgd.com/resources/mp3/d94764f1-9030-4bbb-b63e-bbc997aeaaa5.mp3", _app_string());
			//Http->Get("https://nankgd.com/resources/mp3/06cab439-4d91-4b1d-bc9d-5adbf9e959bb.mp3", _app_string()); 
			...
			/*_https_::Https<_app_string>::HdrList_type	List;
			_https_::Https<_app_string>::tagHdr_INFO	Hdr_INFO;
			Hdr_INFO.Name = "uid";
			Hdr_INFO.Value = "44";
			List.push_back(Hdr_INFO);

			Hdr_INFO.Name = "rcode";
			Hdr_INFO.Value = "DODFZUWPQO0324773I6CCCBL8BSVCLJMY7UYZS5EC2CUVV3RXG";
			List.push_back(Hdr_INFO);
			...
			Http->PostFile("https://nankgd.com/nkytms/uploadIcon", List, "E:\\sip.txt");*/

			_app_string sPath = "E:\\key\\sys.dat";// "/data/sys.dat";
			typedef typename _https_::Https<_app_string>	Https_type;
			...
			Http->PostFile("http://192.168.1.188/nkytms/uploadDeviceFile", List, sPath.c_str());
