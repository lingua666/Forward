...
#include <libFrameWork_RtspClients/RtspClient_Impl_V20.h>
...
RtspClient_Impl_V20::RtspClient_Impl_V20( void )
	: _hNet( -1 )
	, _uSeq( 0 )
	, _SeqCycle( 1 )
	, _uRecvSeq( 0 )
...
int RtspClient_Impl_V20::Init( void )
{
...
void RtspClient_Impl_V20::InitBuf( void )
{
	_MemBuf_sptr->uLen = 4;
	memcpy(_MemBuf_sptr->Buf, H264Header, sizeof(H264Header));
}

void RtspClient_Impl_V20::Release( void )
{
	Close();
}

int RtspClient_Impl_V20::Open( const _string_type& sSessionID, UInt8 uStreamType,
...
int RtspClient_Impl_V20::Play( void )
{
	...
	return GetAsynStreamClientsInstance()->Connect(sIP.c_str(), 
		...
		function20_bind(&RtspClient_Impl_V20::HandleRtspConnect, shared_from_this(), sPath, _function_::_1));
}

void RtspClient_Impl_V20::SetResultEvent( const HFNExceptNT& hExcepNT, const HFNSuccessNT& hSuccessNT )
{
	_CtrlInfo.hExcepNT = hExcepNT;
	_CtrlInfo.hSuccessNT = hSuccessNT;
}

void RtspClient_Impl_V20::SetDataEvent( const HFNSdpNT& hSdpNT, const HFNRtpDataNT& hRtpDataNT )
{
	_CtrlInfo.hRtpDataNT = hRtpDataNT;
	_CtrlInfo.hSdpNT = hSdpNT;
}

void RtspClient_Impl_V20::Close( void )
{
	NETHANDLE h = -1;
	_Lock.Lock();
	...
	if( h != -1 )
	{
		...
		LOG_Print_Info(libFrameWork_RtspClients, "RtspClient_Impl_V20::Close()\r\n")
...
RtspClient_Impl_V20::tagRtspCtrlInfo& RtspClient_Impl_V20::GetRtspCtrlInfo( void )
{
	return _CtrlInfo;
}

void RtspClient_Impl_V20::HandleRtspConnect( const _string_type& sPath, 
	NETHANDLE Handle )
{
	if( Handle == -1 )
	{
	...
	if( _CtrlInfo.uStreamType == RtspPacket::ENUM_UDP_TYPE )
	{
		GetAsynStreamClientsInstance()->SetRecvHandle(_hNet, 
			function20_bind(&RtspClient_Impl_V20::HandleRtspRecv_Udp, shared_from_this(), 
			sPath, _function_::_1, _function_::_2, _function_::_3));
	}
	else
	{
		GetAsynStreamClientsInstance()->SetRecvHandle(_hNet, 
			function20_bind(&RtspClient_Impl_V20::HandleRtspRecv_Tcp, shared_from_this(), 
			sPath, _function_::_1, _function_::_2, _function_::_3));
	}

	if( GetAsynStreamClientsInstance()->SetDestroyHandle(_hNet,
		function20_bind(&RtspClient_Impl_V20::HandleRtspDestroy, shared_from_this(), 
		_function_::_1) ) == -1 )
	{//失败
...
void RtspClient_Impl_V20::HandleRtspRecv_Udp( const _string_type& sPath, 
	NETHANDLE Handle,
	...
	case EN_STATUS_DESCRIBE_ACK:
		{
			if( ParseRet(Stream_sptr->data, Stream_sptr->payload) == "200,OK" )
			{
				...
				GetUDPStreamInstance()->SetRecvHandle(_CtrlInfo.hRtp, 
					function20_bind_Ex(&RtspClient_Impl_V20::HandleRtpRecv, shared_from_this()));

				if( GetUDPStreamInstance()->SetDestroyHandle(_CtrlInfo.hRtp, 
					function20_bind(&RtspClient_Impl_V20::HandleRtpDestroy, shared_from_this(), 
				...
				GetUDPStreamInstance()->SetRecvHandle(_CtrlInfo.hRtcp, 
					function20_bind_Ex(&RtspClient_Impl_V20::HandleRtcpRecv, shared_from_this()));

				if( GetUDPStreamInstance()->SetDestroyHandle(_CtrlInfo.hRtcp, 
					function20_bind(&RtspClient_Impl_V20::HandleRtcpDestroy, shared_from_this(), 
	...
	case EN_STATUS_PLAY_ACK:
		{
			if( ParseRet(Stream_sptr->data, Stream_sptr->payload) == "200,OK" )
			{
				...
				GetTimerEventInstance()->push_back(function20_bind(&RtspClient_Impl_V20::HandleRtpCheck,
...
void RtspClient_Impl_V20::HandleRtspRecv_Tcp( const _string_type& sPath,
	NETHANDLE Handle,
	const StreamBuf_ptr& Stream_sptr,
	long lRemain )
{
...
void RtspClient_Impl_V20::HandleRtpCheck( void )
{
...
void RtspClient_Impl_V20::HandleRtspDestroy( NETHANDLE handle )
{
	LOG_Print_Info(libFrameWork_RtspClients, "RtspClient_Impl_V20::HandleRtspDestroy(%p)\r\n",this)
...
void RtspClient_Impl_V20::HandleRtpRecv( NETHANDLE Node, struct sockaddr_in* pSrc, const _io_net_::StreamBuf_ptr& Buf_ptr )
{
	RtpRecv_Impl(Buf_ptr->data, Buf_ptr->payload);
}

void RtspClient_Impl_V20::RtpRecv_Impl( const char* c_szData, UInt32 uLen )
{
	...
	if( _CtrlInfo.hRtpDataNT )
	{
		...
		if( iLen == 1 )
		{
			if( !_Rtcp.isInit() )
			{
				...
				if( _CtrlInfo.uStreamType == RtspPacket::ENUM_UDP_TYPE )
				{
					GetTimerEventInstance()->push_back( 
						function20_bind(&RtspClient_Impl_V20::SendRtcpRRReport_Udp, shared_from_this(),
						(UInt32)(Timestamp_type().utcTime())), 4000);
				}
				else
				{
					/*GetTimerEventInstance()->push_back( 
						function20_bind(&RtspClient_Impl_V20::SendRtcpRRReport_Tcp, shared_from_this(),
						(UInt32)(Timestamp_type().utcTime())), 4000);*/
				}
			}
			...
			else
			{
				...
				{//出现丢包现象，将已经接收的包丢弃
					//test
					LOG_Print_Info(libFrameWork_RtspClients, "RtspClient_Impl_V20::HandleRtpRecv Lost Seq(%d,%d)", _uRecvSeq, _Rtp.GetHeadSeq());
					printf("RtspClient_Impl_V20::HandleRtpRecv Lost Seq(%d,%d)\r\n", _uRecvSeq, _Rtp.GetHeadSeq());
					//test
			...
			if( !isLost )
			{
				...
				if( PlayloadInfo.isFull )
				{
					if( _MemBuf_sptr->uLen + PlayloadInfo.uDataLen < RTP_MAX_BUF )
					{
						...
						{
							_CtrlInfo.hRtpDataNT(_CtrlInfo.SessionID.c_str(), _MemBuf_sptr);
							_MemBuf_sptr = _MEMBUF_sptr(GetPoolInstance()->AllocObj<_MEMBUF>(),
								function20_bind_Ex(&MemPool_type::FreeObj<_MEMBUF>, GetPoolInstance()));
							_MemBuf_sptr->uLen = 0;
						}
					...
					{
						//test
						LOG_Print_Info(libFrameWork_RtspClients, "RtspClient_Impl_V20::HandleRtpRecv1(%d) Recv Data is more than %d",
				...
				else
				{
					...
					{//超过缓冲区长度
						...
						//test
						LOG_Print_Info(libFrameWork_RtspClients, "RtspClient_Impl_V20::HandleRtpRecv2(%d) Recv Data is more than %d", 
							_MemBuf_sptr->uLen, RTP_MAX_BUF);
						//test
					}
				}
...
void RtspClient_Impl_V20::SendRtcpRRReport_Udp( UInt32 uSSRC )
{
	if( _CtrlInfo.hRtcp != -1 )
	{
...
/*void RtspClient_Impl_V20::SendRtcpRRReport_Tcp( UInt32 uSSRC )
{
	if( _hNet != -1 )
	{
...
void RtspClient_Impl_V20::HandleRtpDestroy( NETHANDLE Node, UInt16 uPort )
{
	LOG_Print_Info(libFrameWork_RtspClients, "RtspClient_Impl_V20::HandleRtpDestroy()\r\n")
}

void RtspClient_Impl_V20::HandleRtcpRecv( NETHANDLE Node, struct sockaddr_in* pSrc, const _io_net_::StreamBuf_ptr& Buf_ptr )
{
	printf("HandleRtcpRecv(%d)\r\n", Buf_ptr->payload);
}

void RtspClient_Impl_V20::HandleRtcpDestroy( NETHANDLE Node, UInt16 uPort )
{
	LOG_Print_Info(libFrameWork_RtspClients, "RtspClient_Impl_V20::HandleRtcpDestroy()\r\n")
}

int	RtspClient_Impl_V20::ParseSdp( const char* c_szData, UInt32 uLen, tagSdpInfo* tagInfo )
{
	char* pEnd = NULL;
...
_string_type  RtspClient_Impl_V20::ParseRet( const char* c_szData, UInt32 uLen )
{
	_string_type s;
...
void RtspClient_Impl_V20::SetSeq( UInt32 uSeq )
{
	_uSeq = uSeq;
}

void RtspClient_Impl_V20::AddSeq( void )
{
	_uSeq ++;
}

UInt32 RtspClient_Impl_V20::GetSeq( void )
{
	return _uSeq;
}
