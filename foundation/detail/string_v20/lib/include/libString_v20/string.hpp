...
#ifndef ___STRING_V20_20160418173307_1438860785_H__
#define ___STRING_V20_20160418173307_1438860785_H__
...
namespace	_string_v20_{
	...
	private:
		_Elem			_Buf[STRING_OBJ_SIZE_V20 + STRING_BUF_SIZE + 1];
		size_type		_Mysize;
		size_type		_Myres;
		...
		{	// insert _Right [_Roff, _Roff + _Count) at _Off
			...
			{	// make room and insert new stuff
				...
				if( this->_Myres < _Num || _Alval.use_count() > 1)
				{
					...
					this->_Myres = _Num + STRING_RESERVED_BUF_SIZE;
					_Alval.swap(tmp);
				}
				else
				{
...
gt_alloc:
		...
		{
			return reinterpret_cast<char*>(_Buf) + STRING_OBJ_SIZE_V20;
		}
	};
	...
	inline string operator+( const _Elem* _Left, const string& _Right )
	{
		string _Ans;
...
#if defined(_SUPPORTED_CXX11_)
	inline string operator+( const _Elem* _Left, string&& _Right )
	{
	...
	inline string operator+( const string& _Left, const _Elem* _Right )
	{
		string _Ans;
...
#if defined(_SUPPORTED_CXX11_)
	inline string operator+( string&& _Left, const _Elem* _Right )
	{
	...
	inline bool operator==( const _Elem* _Left, const string& _Right )
	{
		return (_Right.compare(_Left) == 0);
	}

	inline bool operator==( const string& _Left, const _Elem*_Right )
	{
		return (_Left.compare(_Right) == 0);
	}
...
#include <libString_v20/hash.hpp>
