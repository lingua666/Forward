...
#include <libEPoll_File/CAsynIO.h>
#include <libEPoll_File/io_service.h>
	...
	namespace _epoll_non_regular_file_{
		...
		int io_service::open( void )
		{
			...
			if( _io_handle == NULL )
			{
				LOG_Print_Error(libEPoll_File, "io_service Create Error!");
				return -1;
			}
			return 1;
		}
		...
		{			
			...
			{
				LOG_Print_Error(libEPoll_File, "io_service::run sigaction fail");
				return ;
			} 
					...
					case IO_OP_READ:
						{				
							...
							if( pOver->_ProcRef < 0 && pOver->_hfnComplete )
							{
								...
								pOver->_SWSAOverlapp.wsaOverlapped.uOP, uOP, pOver->_ADR._Handle);
								_exit(0);
							}
								...
								{//socket已经关闭
									if( pOver->_ProcRef <= 0 )
									{
										if( pOver->_ProcRef < 0 )
										{
											...
											pOver->_SWSAOverlapp.wsaOverlapped.uOP, pOver->_ADR._Handle);
											_exit(0);
										}
					...
					case IO_OP_WAIT:
						{										
							...
							{//出现错误
								...
								{
									LOG_Print_Info(libEPoll_File, "io_service::run GetQueuedCompletionStatus failed, error code:%d!!", errno)
										break;//退出线程
								}
							}
						}
		...
		void io_service::process( tagIOData_INFO* pInfo, tagOverlapped* lpOverlapped, UInt32 Event )
		{
			...
			{					
				...
				if( hComplete )
				{
					...
					{
						lpOverlapped->_RealSize = lpOverlapped->_hfnPerform(lpOverlapped->_ADR._Handle,
			...
			//LOG_Print_Info(libEPoll_File, "io_service::process end")
		}
