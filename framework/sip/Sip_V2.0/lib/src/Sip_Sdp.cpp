...
namespace	_sip_v20_ {
	...
	void Sip_SDP::Init_sdp_session(sdp_session* pSdp)
	{
	...
	Sip_SDP::Sip_SDP(const self_type& t1)
	{
		Init();
		Copy(t1._Sdp);
	}
	...
	Sip_SDP::self_type&	Sip_SDP::operator=(const self_type& t1)
	{
		Copy(t1._Sdp);
		return *this;
	}

	int Sip_SDP::Init(void)
	{
		Init_sdp_session(&_Sdp);
		return 1;
	}

	void Sip_SDP::Release(void)
	{
		for (int i = 0; i < this->_Sdp.bandw_count; i++)
		{
			this->_Sdp.bandw[i].reset();
		}

		for (int i = 0; i < this->_Sdp.attr_count; i++)
		{
			this->_Sdp.attr[i].reset();
		}

		for (int i = 0; i < this->_Sdp.media_count; i++)
		{
			this->_Sdp.media[i].reset();
		}

		this->_Sdp.proto_version = 0;
		this->_Sdp.attr_count = 0;
		this->_Sdp.bandw_count = 0;
		this->_Sdp.media_count = 0;
		this->_Sdp.origin.id = 0;
		this->_Sdp.origin.version = 0;
		this->_Sdp.time.start = 0;
		this->_Sdp.time.stop = 0;
	}

	void Sip_SDP::Copy(const sdp_session& Session)
	{
		Release();

		this->_Sdp.proto_version = Session.proto_version;
		this->_Sdp.origin.user = Session.origin.user;
		this->_Sdp.origin.id = Session.origin.id;
		this->_Sdp.origin.version = Session.origin.version;
		this->_Sdp.origin.net_type = Session.origin.net_type;
		this->_Sdp.origin.addr_type = Session.origin.addr_type;
		this->_Sdp.origin.addr = Session.origin.addr;
		this->_Sdp.name = Session.name;

		this->_Sdp.session_info = Session.session_info;
		this->_Sdp.conn = Session.conn;
		this->_Sdp.bandw_count = Session.bandw_count;
		for (int i = 0; i < this->_Sdp.bandw_count; i++)
		{
			this->_Sdp.bandw[i] = Session.bandw[i];
		}

		this->_Sdp.time.start = Session.time.start;
		this->_Sdp.time.stop = Session.time.stop;

		this->_Sdp.attr_count = Session.attr_count;
		for (int i = 0; i < this->_Sdp.attr_count; i++)
		{
			this->_Sdp.attr[i] = Session.attr[i];
		}

		this->_Sdp.media_count = Session.media_count;
		for (int i = 0; i < this->_Sdp.media_count; i++)
		{
			this->_Sdp.media[i] = Session.media[i];
		}
	}
	...
	const Sip_SDP::sdp_session* Sip_SDP::Parse(const char* c_szData, UInt32 uSize)
	{
		int iOffset = 0, iLen = uSize;
		char* pCur = const_cast<char*>(c_szData);

		do
		{
			iOffset = ParseLine(pCur, iLen, &_Sdp);
			iLen -= iOffset;
			if (iOffset == -1 || iLen <= 0)
			{
				break;
				...
				case 'm':
					{
						...
						if (pSdp->media_count < SDP_MEDIA_MAX)
						{
							...
							if (sPort == "")
							{//不存在PortCount
								if (sPortAndCount == "0" || sPortAndCount == "-")
									media->desc.port = 0;
								else
	...
	const Sip_SDP::sdp_session* Sip_SDP::Get(void) const
	{
		return &_Sdp;
	}

	_string_t Sip_SDP::toString(void)
	{
		const char* End = "\r\n";
		...
		//v=
		str = "v=" + _string_t::NumberToStr(this->_Sdp.proto_version) + End;

		//o=
		str += "o=" + this->_Sdp.origin.user;
		str += " " + _string_t::NumberToStr(this->_Sdp.origin.id);
		str += " " + _string_t::NumberToStr(this->_Sdp.origin.version);
		str += " " + this->_Sdp.origin.net_type;
		str += " " + this->_Sdp.origin.addr_type;
		str += " " + this->_Sdp.origin.addr + End;

		//s=
		if(this->_Sdp.name != "")
			str += "s=" + this->_Sdp.name + End;

		//i=
		if (this->_Sdp.session_info)
		{
			str += "i=" + this->_Sdp.session_info->author + " " + this->_Sdp.session_info->copyright + End;
		}

		//c=
		if (this->_Sdp.conn)
		{
			str += "c=" + this->_Sdp.conn->net_type;
			str += " " + this->_Sdp.conn->addr_type;
			str += " " + this->_Sdp.conn->addr + End;
		}

		int i = 0;
		//b=
		for (i = 0; i < this->_Sdp.bandw_count; i ++)
		{
			if(!this->_Sdp.bandw[i])
				continue;

			str += "b=" + this->_Sdp.bandw[i]->modifier + ":";
			str += _string_t::NumberToStr(this->_Sdp.bandw[i]->value) + End;
		}

		//t=
		str += "t=" + _string_t::NumberToStr(this->_Sdp.time.start) + " " + _string_t::NumberToStr(this->_Sdp.time.stop) + End;

		//a=
		for (i = 0; i < this->_Sdp.attr_count; i++)
		{
			if (!this->_Sdp.attr[i])
				continue;

			str += "a=" + this->_Sdp.attr[i]->name;
			if (this->_Sdp.attr[i]->value != "")
			{
				str += ":";
				str += this->_Sdp.attr[i]->value;
			}
		...
		//m=
		for (i = 0; i < this->_Sdp.media_count; i++)
		{
			if(!this->_Sdp.media[i])
				continue;

			int j = 0;
			str += "m=" + this->_Sdp.media[i]->desc.media;

			if (this->_Sdp.media[i]->desc.port == 0)
				str += " -";
			else
				str += " " + _string_t::NumberToStr(this->_Sdp.media[i]->desc.port);

			if (this->_Sdp.media[i]->desc.port_count > 0)
			{
				str += "/" + _string_t::NumberToStr(this->_Sdp.media[i]->desc.port_count);
			}

			str += " " + this->_Sdp.media[i]->desc.transport;
			for (j = 0; j < this->_Sdp.media[i]->desc.fmt_count; j++)
			{
				str += " " + this->_Sdp.media[i]->desc.fmt[j];
			}

			str += End;

			for (j = 0;  j < this->_Sdp.media[i]->attr_count; j ++)
			{
				str += "a=" + this->_Sdp.media[i]->attr[j]->name;
				if (this->_Sdp.media[i]->attr[j]->value != "")
				{
					str += ":";
					str += this->_Sdp.media[i]->attr[j]->value;
				}

				str += End;
			}

			for (j = 0; j < this->_Sdp.media[i]->bandw_count; j++)
			{
				str += "b=" + this->_Sdp.media[i]->bandw[j]->modifier + ":";
				str += _string_t::NumberToStr(this->_Sdp.media[i]->bandw[j]->value) + End;
			}
		}
