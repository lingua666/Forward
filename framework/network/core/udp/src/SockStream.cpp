...
namespace _io_net_{
	...
	namespace _udp_{
		...
		void	SockStream::HandleRecv( tagIOData_INFO* pIOData )
		{
			...
			EVENT_EXEC_READ(shared_from_this(), (struct sockaddr_in*)pOver->_pAddr,
				StreamBuf_ptr(StreamBuf::GetHeadToData(tmBuf), function20_bind_Ex(&MemPool_type::Free, &_Pool)) );
			//数据处理
		...
		void	SockStream::PostSend( sockaddr_in* pAddr, const char* c_szData, UInt32 u32Size )
		{
			if( !_isOpen )
				return ;

			if( _Socket.AsyncSend( pAddr, c_szData, u32Size, 
		...
		void	SockStream::HandleSend( const char* pBuf, tagIOData_INFO* pIOData )
		{
			...
			{
				EVENT_EXEC_WRITE(shared_from_this(), (struct sockaddr_in*)pOver->_pAddr, pBuf, pIOData->_ibytes_transferred);
			}
		}
