...
#include <libFrameWork_Rtsp/Nalu265.h>

h265_sei_t* h265_sei_new()
{
	h265_sei_t* s = (h265_sei_t*)MemoryAllocator::Alloc(sizeof(h265_sei_t));
...
{
	if (s == NULL) return;
	if ( s->payload != NULL ) MemoryAllocator::Free(s->payload);
	MemoryAllocator::Free(s);
}


Nalu265::Nalu265( void )
{
	memset(&_Head, 0, sizeof(_Head));
	memset(&_VedioInfo, 0, sizeof(_VedioInfo));
}

h265_stream_t* Nalu265::h265_new()
{
	h265_stream_t* h = (h265_stream_t*)MemoryAllocator::Calloc(sizeof(h265_stream_t));

	h->nal = (h265_nal_t*)MemoryAllocator::Calloc(sizeof(h265_nal_t));

	// initialize tables
	for ( int i = 0; i < 16; i++ ) { h->vps_table[i] = (h265_vps_t*)MemoryAllocator::Calloc(sizeof(h265_vps_t)); }
	for ( int i = 0; i < 32; i++ ) { h->sps_table[i] = (h265_sps_t*)MemoryAllocator::Calloc(sizeof(h265_sps_t)); }
	for ( int i = 0; i < 256; i++ ) { h->pps_table[i] = (h265_pps_t*)MemoryAllocator::Calloc(sizeof(h265_pps_t)); }

	h->vps = h->vps_table[0];
	h->sps = h->sps_table[0];
	h->pps = h->pps_table[0];
	h->aud = (h265_aud_t*)MemoryAllocator::Calloc(sizeof(h265_aud_t));
	h->num_seis = 0;
	h->seis = NULL;
	h->sei = NULL;  //This is a TEMP pointer at whats in h->seis...
	h->sh = (h265_slice_header_t*)MemoryAllocator::Calloc(sizeof(h265_slice_header_t));

	h->info = (videoinfo_t*)MemoryAllocator::Calloc(sizeof(videoinfo_t));
	h->info->type = 1;
	return h;
}
...
void Nalu265::h265_free(h265_stream_t* h)
{
	MemoryAllocator::Free(h->nal);
	for ( int i = 0; i < 16; i++ ) { if (h->vps_table[i]!=NULL) MemoryAllocator::Free( h->vps_table[i] ); }
	for ( int i = 0; i < 32; i++ ) { if (h->sps_table[i]!=NULL) MemoryAllocator::Free( h->sps_table[i] ); }
	for ( int i = 0; i < 256; i++ ) { if (h->pps_table[i]!=NULL) MemoryAllocator::Free( h->pps_table[i] ); }

	if (h->aud != NULL)
	{
		MemoryAllocator::Free(h->aud);
	}
	...
	if(h->seis != NULL)
	{
		...
		MemoryAllocator::RFree(h->seis);
	}
	MemoryAllocator::Free(h->sh);
	MemoryAllocator::Free(h);
}

int Nalu265::Init( const char* c_szData, UInt32 uLen )
{
	bs_t b ;
	bs_init(&b, (uint8_t*)c_szData, uLen);

	// nal header
	_Head.forbidden_zero_bit = bs_read_f(&b,1);
	_Head.nal_unit_type = bs_read_u(&b,6);
	_Head.nuh_layer_id = bs_read_u(&b,6);
	_Head.nuh_temporal_id_plus1 = bs_read_u(&b,3);
	_Head.parsed = NULL;
	_Head.sizeof_parsed = 0;
...
int Nalu265::GetSPS( h265_sps_t* pSPS )
{
	if( _Head.nal_unit_type != NAL_UNIT_SPS )
		return -1;

	bs_t b ;
	bs_init(&b, (uint8_t*)_sRbsp.c_str(), _sRbsp.size());
	h265_read_sps_rbsp(pSPS, &b);
	return 1;
}

int Nalu265::GetPPS( h265_pps_t* pPPS )
{
	if( _Head.nal_unit_type != NAL_UNIT_PPS )
		return -1;

	bs_t b ;
	bs_init(&b, (uint8_t*)_sRbsp.c_str(), _sRbsp.size());
	h265_read_pps_rbsp(pPPS, &b);
	return 1;
}

bool Nalu265::isSlice( void )
{
	return _Head.nal_unit_type == NAL_UNIT_CODED_SLICE_TRAIL_R  
		|| _Head.nal_unit_type == NAL_UNIT_CODED_SLICE_TRAIL_N  
		|| _Head.nal_unit_type == NAL_UNIT_CODED_SLICE_TSA_N  
		|| _Head.nal_unit_type == NAL_UNIT_CODED_SLICE_STSA_R  
		|| _Head.nal_unit_type == NAL_UNIT_CODED_SLICE_STSA_N  
		|| _Head.nal_unit_type == NAL_UNIT_CODED_SLICE_BLA_W_LP  
		|| _Head.nal_unit_type == NAL_UNIT_CODED_SLICE_BLA_W_RADL  
		|| _Head.nal_unit_type == NAL_UNIT_CODED_SLICE_BLA_N_LP  
		|| _Head.nal_unit_type == NAL_UNIT_CODED_SLICE_IDR_W_RADL  
		|| _Head.nal_unit_type == NAL_UNIT_CODED_SLICE_IDR_N_LP  
		|| _Head.nal_unit_type == NAL_UNIT_CODED_SLICE_CRA  
		|| _Head.nal_unit_type == NAL_UNIT_CODED_SLICE_RADL_N  
		|| _Head.nal_unit_type == NAL_UNIT_CODED_SLICE_RADL_R  
		|| _Head.nal_unit_type == NAL_UNIT_CODED_SLICE_RASL_N  
		|| _Head.nal_unit_type == NAL_UNIT_CODED_SLICE_RASL_R;  
}

UInt8 Nalu265::Get_Type( const char* c_szData, UInt32 uLen )
{
	h265_nal_t	Head;
	bs_t b ;
	bs_init(&b, (uint8_t*)c_szData, uLen);
	bs_read_u(&b,1);
	return bs_read_u(&b,6);
}

// 7.4.1.1 Encapsulation of an SODB within an RBSP
_string_type Nalu265::rbsp_to_nal(const uint8_t* rbsp_buf, const int* rbsp_size)
{
	_string_type s;
	int i = 0, count = 0;
...
// 7.4.1.1 Encapsulation of an SODB within an RBSP
_string_type Nalu265::nal_to_rbsp(const int nal_header_size, const uint8_t* nal_buf, int* nal_size)
{
	_string_type s;
	s.reserve(*nal_size);
...
void h265_read_scaling_list(scaling_list_data_t* sld, bs_t* b)
{
	for(int sizeId = 0; sizeId < 4; sizeId++)
	{
		for(int matrixId = 0; matrixId < 6; matrixId += ( sizeId == 3 ) ? 3 : 1)
		{
			...
			{
				int nextCoef = 8;
				int coefNum = __min(64, (1 << (4 + (sizeId << 1))));
...
void  Nalu265::h265_read_vps_rbsp(h265_stream_t* h, bs_t* b)
{
	int i = 0;
	int j = 0;
...
void Nalu265:: h265_read_sps_rbsp(h265_sps_t* sps, bs_t* b)
{
	...
	_VedioInfo.chroma_format_idc = sps->chroma_format_idc;
	...
	_VedioInfo.width  = sps->pic_width_in_luma_samples;
	_VedioInfo.height = sps->pic_height_in_luma_samples;
	...
	if (sps->conformance_window_flag)
	{
		...
		_VedioInfo.crop_left = sps->conf_win_left_offset;
		_VedioInfo.crop_right = sps->conf_win_right_offset;
		_VedioInfo.crop_top = sps->conf_win_top_offset;
		_VedioInfo.crop_bottom = sps->conf_win_bottom_offset;
		...
		_VedioInfo.width  -= (sub_width_c*sps->conf_win_right_offset + sub_width_c*sps->conf_win_left_offset);
		_VedioInfo.height -= (sub_height_c*sps->conf_win_bottom_offset + sub_height_c*sps->conf_win_top_offset);
	}
	...
	// bit depth
	_VedioInfo.bit_depth_luma = sps->bit_depth_luma_minus8 + 8;
	_VedioInfo.bit_depth_chroma = sps->bit_depth_chroma_minus8 + 8;
	...
	if (sps->vui_parameters_present_flag)
	{
		...
		if (sps->vui.vui_num_units_in_tick != 0)
			_VedioInfo.max_framerate = (float)(sps->vui.vui_time_scale) / (float)(sps->vui.vui_num_units_in_tick);
	}
...
void Nalu265::h265_read_pps_rbsp(h265_pps_t* pps, bs_t* b)
{
	...
	_VedioInfo.encoding_type = pps->entropy_coding_sync_enabled_flag;
...
void Nalu265::h265_read_slice_layer_rbsp(h265_stream_t* h, bs_t* b)
{
...
void Nalu265::h265_read_aud_rbsp(h265_stream_t* h, bs_t* b)
{
...
void Nalu265::h265_read_end_of_seq_rbsp(h265_stream_t* h, bs_t* b)
{
}

//7.3.2.7 End of stream RBSP syntax
void Nalu265::h265_read_end_of_stream_rbsp(h265_stream_t* h, bs_t* b)
{
}
...
static void read_user_data_unregistered(h265_stream_t* h, bs_t* b, int payloadSize)
{
	...
	s->payload = (uint8_t*)MemoryAllocator::Alloc(payloadSize);
...
void Nalu265::h265_read_sei_rbsp(h265_stream_t* h, bs_t* b)
{
	//return;
	for (int i = 0; i < h->num_seis; i++)
	{
	...
	do {
		h->num_seis++;
		h->seis = (h265_sei_t**)MemoryAllocator::Realloc(h->seis, h->num_seis * sizeof(sei_t*));
