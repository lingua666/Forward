...
namespace	_https_ {
	...
	public:
		...
		typedef function20_handle<void(StatusCode, const char*, UInt32)>	HFNEvent;
	...
	protected:
			...
			const _http_string& sContentType)
		{
			...
			{//发现错误
				...
				if (_hEvent)
					_hEvent(EN_Status_Err_Connect, NULL, 0);
				return iRet;
			}
		...
		void HandleDestroy(NETHANDLE hNet)
		{
			Close();
			if (_hEvent)
				_hEvent(EN_Status_Close, NULL, 0);
		...
		void HandleSSLEvent(_openssl_::Ssl::SSL_STATUS Status, void* pParamer)
		{
			...
			case _openssl_::Ssl::EN_Status_Err_Encode:
			{
				if (_hEvent)
				{
					_hEvent(EN_Status_Err_Encode, NULL, 0);
				}
			}
			break;
			case _openssl_::Ssl::EN_Status_Err_Decode:
			{
				if (_hEvent)
				{
					_hEvent(EN_Status_Err_Decode, NULL, 0);
				}
			}
			break;
			case _openssl_::Ssl::EN_Status_Err_Send:
			{
				if (_hEvent)
				{
					_hEvent(EN_Status_Err_Send, NULL, 0);
				}
			}
			break;
			}
			...
			typename _http_::Http<_http_string>::StatusCode Code, void* pParam)
		{
			...
			{//准备接收数据
				if (_hEvent)
				{
					_hEvent(EN_Status_Data_Start, NULL, 0);
				}
				return;
			}
			break;
			...
			{//接收到数据
				...
				if (_hEvent)
				{
					_hEvent(EN_Status_Data, pData->pBuf, pData->uLen);
				}
				return;
			}
			break;
			...
			{//数据接收完成
				if (_hEvent)
				{
					_hEvent(EN_Status_Data_End, NULL, 0);
				}
				Close();
			}
			break;
			...
			{//成功
				if (_hEvent)
				{
					_hEvent(EN_Status_Data, (char*)pParam, strlen((char*)pParam));
				}
			}
			break;
			...
			const tagHttp_INFO& Info, const HdrList_type& Hdrs, const _http_string& sFilePath)
		{
			...
			{//握手成功
				...
				if (iRet == -1)
				{
					LOG_Print_Error(libFrameWork_Https, "Http::HandleConnectFile(%s:%d) Failed!!", Info.sUrl.c_str());
				}
				return 1;
			}
			...
			const _http_string& sFilePath)
		{
			...
			int iRet = _Ssl.Encode(sContent.c_str(), sContent.size());
			if (iRet == -1)
				return iRet;
			...
			if (_hFile != NULL)
			{
				...
				return _Ssl.Encode(szData, iCount);
			}
		...
		void HandleSend(const _http_string& sBoundary, const char* pHasData, UInt32 HasSize)
		{
			if (_hFile != NULL)
			{
				...
				{//读取文件完毕
					...
					_http_string End = "\r\n" + sBoundary + "--\r\n";
					_Ssl.Encode(End.c_str(), End.size());
		...
		void HandleCloseFile(void)
		{
			...
			if (_hEvent)
			{
				_hEvent(EN_Status_UpLoad, NULL, 0);
			}
		}
