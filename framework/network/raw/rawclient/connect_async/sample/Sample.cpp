...
_client_::_async_::RawClients	Clients;
...
int main(int argc, char* argv[])
{
	...
	Clients.Init(get_processor_number() * 2 + 2);
	...
	while(true)
	{
		...
		for( i = 0; i < iNum; i ++ )
		{
			...
			if( Handle[i] != 0 )
			{
				...
				Clients.SetCloseHandle(Handle[i], function20_bind(ClientClose, GetIPv4ToString(Handle[i]),
														GetPort(Handle[i]),_function_::_1));
			}
		}
