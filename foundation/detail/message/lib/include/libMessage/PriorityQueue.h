...
#ifndef __PRIORITYQUEUE_20150803173634_1438594594_H__
#define __PRIORITYQUEUE_20150803173634_1438594594_H__

#include <libContainer/Queue.hpp>
#include <libLock/Lock.hpp>

//优先级别允许设置最高为5级   数值越大，优先级别越大
#define	MESSAGE_PRIORITY_NUM	5

namespace _priority_queue_{
...
{
public:
	typedef PriorityQueue<T>					self_type;
	typedef	T									value_type;
	typedef	UInt32								size_type;
	typedef const value_type&					const_reference;
	typedef typename _priority_queue_::iterator<T>			iterator;
	typedef typename _priority_queue_::iterator<T>::_Exiter	_Exiter;
...
public:
	...
	*/
	...
	inline bool push_back( const value_type& t1, UInt8 u8Priority = 0 )
	{
		u8Priority = __min(u8Priority, MESSAGE_PRIORITY_NUM - 1);

		_Queue[u8Priority].push_back(t1);
...
start:
		...
		return _Queue[_CurPriority].front();
	}
...
start:
	...
	{	// subscript nonmutable sequence
		if( Pos > size() - 1 )
			return _container_::QueueObj<value_type>::RetNull();
...
gt_start:
		if( GetFlag(Priority) && !_Queue[Priority].empty() )
		{
			...
			{
				const_reference c_rf = _Queue[Priority][Pos];
				return c_rf;
			}
	...
	{
		if( u8Priority < MESSAGE_PRIORITY_NUM )
		{
	...
	inline void clear( void )
	{
		...
		for( int i = 0; i < MESSAGE_PRIORITY_NUM; i ++ )
		{
			_Queue[i].clear();
		}
	}
...
start:
	...
	{
		if( u8Priority > MESSAGE_PRIORITY_NUM - 1 || u8Priority <= 0 )
			return 0;
...
private:
	Queue_type< value_type, CNoneLock, MemPool_macro<0, CNoneLock> >	_Queue[MESSAGE_PRIORITY_NUM];
	UInt8 _CurPriority;
	UInt8 _UseFalg;
	UInt32	_u32Size;
	//CLock	_Lock;
