...
namespace	_sip_ {
	...
	{
		return _uuid_::UUIDGenerator().create().toString().c_str();
	}
	...
	_string_t  ISip_Proto::GenerateBranch(void)
	{
		...
		Branch += _uuid_::UUIDGenerator().create().toString().c_str();
		return Branch;
	}
	...
	void	ISip_Proto::SetContent(const _string_t& Content)
	{
		_Content = Content;
	}
	...
	_string_big_t	ISip_Proto::GenerateString(const _string_t& Head)
	{
		...
		if (_Content.size() > 0)
		{
			str += _string_t::NumberToStr(_Content.size() + 2).c_str();	//加上"\r\n"
			str += "\r\n\r\n";
			str += _Content.c_str();
			str += "\r\n";
		}
