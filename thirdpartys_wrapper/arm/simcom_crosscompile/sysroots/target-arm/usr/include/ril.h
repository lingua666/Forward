...
#ifndef RIL_INT32_MAX
#define RIL_INT32_MAX (2147483647)
#endif
...
typedef enum {
    ...
    RIL_E_INVALID_PARAMETER = 28,                /* for NFC smartcard API 2.3.2 */
	RIL_E_NOT_READY = 29,						 /* function is not ready */
...
typedef enum {
    ...
    PREF_NET_TYPE_LTE_GSM_WCDMA            = 9, /* LTE, GSM/WCDMA (auto mode)*/
    PREF_NET_TYPE_LTE_CMDA_EVDO_GSM_WCDMA  = 10, /* LTE, CDMA, EvDo, GSM/WCDMA */
    PREF_NET_TYPE_LTE_ONLY                 = 11,
    PREF_NET_TYPE_LTE_WCDMA                = 12,
    PREF_NET_TYPE_LTE_GSM_GSM_PREF         = 13,
    PREF_NET_TYPE_LTE_GSM_LTE_PREF         = 14,
    PREF_NET_TYPE_LTE_GSM                  = 15,
    PREF_NET_TYPE_LTE_WCDMA_WCDMA_PREF     = 16,
    PREF_NET_TYPE_LTE_WCDMA_LTE_PREF       = 17,
    PREF_NET_TYPE_LTE_GSM_WCDMA_GSM_PREF   = 19,
    PREF_NET_TYPE_LTE_GSM_WCDMA_WCDMA_PREF = 20,
    PREF_NET_TYPE_LTE_GSM_WCDMA_LTE_PREF   = 21,
...
} __attribute__ ((packed))  RIL_UUS_Info;
...
typedef enum _RIL_CallMode{
    RIL_CALL_MODE_VOICE = 0,               				/**< Voice only */
    RIL_CALL_MODE_DATA,                    				/**< Data only */
    RIL_CALL_CMODE_FAX,                     				/**< Fax only */
    RIL_CALL_MODE_VOICE_FB_DATA_IS_VOICE,  		/**< Voice followed by data, voice mode */
    RIL_CALL_MODE_ALT_VOICE_DATA_IS_VOICE, 	/**< Alternating voice/data, voice mode */
    RIL_CALL_MODE_ALT_VOICE_FAX_IS_VOICE,  		/**< Alternating voice/fax, voice mode  */
    RIL_CALL_MODE_VOICE_FB_DATA_IS_DATA,   		/**< Voice followed by data, data mode  */
    RIL_CALL_MODE_ALT_VOICE_DATA_IS_DATA,  	/**< Alternating voice/data, data mode  */
    RIL_CALL_MODE_ALT_VOICE_FAX_IS_FAX,    		/**< Alternating voice/fax, fax mode    */
    RIL_CALL_MODE_UNKNOWN,  /**< Unknown call mode */
    #ifdef SS_IPC_SUPPORT
	RIL_CALL_MODE_EMERGENCY, /**< Emergency call mode */
	#endif
} RIL_CallMode;

typedef enum _RIL_ACT_MODE{
	RIL_ACTMODE_NO_SRV =0,
	RIL_ACTMODE_CDMA,
    RIL_ACTMODE_GSM,   
    RIL_ACTMODE_UMTS, 
    RIL_ACTMODE_LTE,
    ...
    RIL_NUM_ACTMODE             /* Number of call act modes defined */
} RIL_ActMode;

typedef enum _RIL_ALERTING_TYPE{
...
typedef struct {
	...
	char			isEmergency; /* is emergency call or not */
	RIL_ActMode		actmode;	
	RIL_AlertType   alerttype;
	RIL_CallMode    mode; 		/* call mode */
...
} __attribute__ ((packed)) RIL_Call_Ext;
...
typedef struct {
	...
	char *			subnetmask;  /*netmask, current only support v4*/								   
...
typedef enum {
   ...
   /* reasons for data call drop - network/modem disconnect */
	...
	PDP_FAIL_CID_EXIST_FAIL = -7,		  /* cid is exist already*/
...
typedef enum {
    ...
    RIL_DC_POWER_STATE_UNKNOWN  = RIL_INT32_MAX // Unknown state
} RIL_DcPowerStates;
...
typedef struct EEMGINFO_s
{
	int state;
	int network_type;
} EEMGinfo;

typedef struct rilDevGsmServingCellInfo_s {
    int mcc;
	int lenOfMnc;
	int mnc;
	int lac;
	...
	int EGPRSSupport;
    int nccPermitted;
	int RadioLinkTimeout;
	int hoCount;
	int hoSuccessCount;
	int chanAssCount;
	int chanAssSuccessCount;
	int gsmBand;
	int channelMode;
}rilDevGsmServingCellInfo;


typedef struct rilDevGPRSPTMInfo_s{
    int GPRSAttached;
    int GPRSAttachType;
    int gprsServiceType;
    int txPower;
    ...
    int ulMod;
    int dlMod;
    int egprsBep_gmskCvBep;
    int egprsBep_eightPskCvBep;
    int egprsBep_gmskMeanBep;
    int egprsBep_eightPskMeanBep;
    int EGPRSBEPPeriod;
    int IsSingleGmmRejectCause;
    int NumActivePDPContext;
    int MacMode;
    int NetworkControl;
    int NetworkMode;
    int EGPRSLQMeasurementMode;
    int egprsBep_status;	
}rilDevGPRSPTMInfo;

typedef struct rilDevGsmNeighboringCellInfo_s
{
    int cellnum;
    int mcc;         				
    int mnc;         				
    ...
    int C31;         				
    int C32;         				
}rilDevGsmNeighboringCellInfo;

typedef struct rilDevEEMGINBFTM_s{
    int EngModeRepOpt;
    int mcc;
    int mnc;
    int lac;
...
typedef struct rilDevGsmGMMInfo_s
{
	int MccLastRegisteredNetwork;	
	int MncLastRegisteredNetwork;	
 	int TMSI;						
	int	PTMSI;						
    int IsSingleMmRejectCause;		
 	int MMRejectCause;				
 	int currentBandMode;			
	int mmState;					
 	int gmmState;					
 	int gprsReadyState;				
 	int readyTimerValueInSecs;		
    int serviceStatus;              
    int LAU_status;                 
    int LAU_count;                  
    int RAU_status;                 
...
typedef struct rilDevLteEngScellParams_s
{
    int mcc;
    int lenOfMnc;   
    int mnc;       
    ...
    int dlBandwidth;
    int cellId;
	int subFrameAssignType;
	int specialSubframePatterns;
	int transMode;
}rilDevLteEngScellParams;

typedef struct rilDevLteEngScellMeas_s
{
    int rsrp;
    int rsrq;
    int sinr;
    ...
    int diversityRsrp; 
    int mainRsrq; 
    int diversityRsrq; /**< Rsrq of diversity antenna. Value range: 0~34, invalid(0xFF) */
    int rssi; /**< Value range: 0~128, invalid(0xFF). (rssi-128) for dbm format */
    int cqi; /**< Value range: 0~15, invalid(0xFFFF). */
	int pathLoss;
	int tb0DlTpt;
	int tb1DlTpt;
	int tb0DlPeakTpt;
    ...
    int tb0UlPeakTpt;
    int tb1UlPeakTpt;
	int dlThroughPut;     //dl throughtput unit bps, sum(dl tb size in bits)/dl statistic time duration
	int dlPeakThroughPut; //Kbps
	int averDlPRB;
	int averCQITb0;
	int averCQITb1;
	int rankIndex;
	int grantTotal;        //total received grant size in unit of BYTE
	int ulThroughPut; //Kbps
	int ulPeakThroughPut; //Kbps
	int currPuschTxPower;
	int averUlPRB;
    int reserved0;
    int dlBler;
    int ulBler;
	...
	int reserved3;
	int reserved4;
}rilDevLteEngScellMeas;

typedef struct rilDevLteUeOperationStatus_s
{
    int errcModeState;      /**< ERRC state \sa CiDevLteEngModeState */
    int emmState;           /**<  EMM state  */  /* see enum EmmState in Mmr_sig.h */
    int serviceState;       /**<  Service state  */  /* see enum ServiceStatus in Mmr_sig.h */
    int IsSingleEmmRejectCause;		/**<  TRUE - only one EMM reject cause reported during the last engineering information period; FALSE - other \sa CCI API Ref Manual  */
    int EMMRejectCause;             /**<  Reported during the last engineering information period  */
    int mmeGroupId;         /**< A member of GUTI */
    int mmeCode;            /**< A member of GUTI */
    int mTmsi;              /**< A member of GUTI */
} rilDevLteUeOperationStatus;

typedef struct rilDevLteEngScellInfo_s
{
    rilDevLteEngScellParams       params;
    rilDevLteEngScellMeas         meas;
    rilDevLteUeOperationStatus    ueOpStatus;
}rilDevLteEngScellInfo;

typedef struct rilDevLteEngIntraFreqNcellInfo_s
{
    int index;
    int phyCellId;
    int euArfcn;
    ...
    int mnc;        /**< 2-3 digits of MNC */
    int tac;
    int cellId;
}rilDevLteEngIntraFreqNcellInfo;

typedef struct rilDevLteEngInterFreqNcellInfo_s
{
    int index;
    int phyCellId;
    int euArfcn;
    ...
    int tac;
    int cellId;
}rilDevLteEngInterFreqNcellInfo;

typedef struct rilDevUmtsGsmNeighborInfo_s
{
    int index;
    /* Measurements */
    int gsmRssi;         	/**< GSM carrier RSSI */
    int rxLev;             	/**< Cell selection Rx level */
    int C1;                  /**< Path loss criterion parameter #1 */
    int C2;                  /**< Path loss criterion parameter #2 */
    ...
    int bsic;		   	    /**< Base transceiver station identity code; range 0h-3Fh (6 bits) */
    int res1U8;
} rilDevUmtsGsmNeighborInfo;

typedef struct rilDevUmtsFddTddNeighborInfo_s
{
    int index;
    /* Measurements */
    int rscp;        	    /**< CPICH/PCCPCH received signal code power */
    int utraRssi;           	/**< UTRA carrier RSSI */
    int cpichEcN0;        	/**< CPICH Ec/N0, only valid for FDD */
    int sQual;              	/**< Cell selection quality (Squal), only valid for FDD  */
    int sRxLev;            	/**< Cell selection Rx level (Srxlev) */
    ...
    int ci;                  	/**< Cell Identity */
    int arfcn;            	/**< Absolute radio frequency channel number */
    int psc_cellParameterId; /**< Primary scrambling code for FDD or Cell parameter id for TDD */
} rilDevUmtsFddTddNeighborInfo;

typedef struct rilDevUmtsServingCellMeasurements_s
{
    int rscp;        	/**< CPICH/PCCPCH received signal code power; in UMTS FDD/TDD messages RSCP is
    ...
    int utraRssi;         	/**< 	UTRA Carrier RSSI; range 0 - 63
    ...
    int sQual;            	/**< Cell selection quality (Squal), only valid for FDD */
    int sRxLev;           	/**< Cell selection Rx level (Srxlev) */
    int txPower;         		/**< UE transmitted power */

/*  TBD: Transport Channel BLER */

} rilDevUmtsServingCellMeasurements;

typedef struct rilDevUmtsServingCellParameters_t
{
    int rac;              			/**< Routing area code */
    int nom;              			/**< Network operation mode */

    int mcc;              			/**< Mobile country code */
    int mnc;              			/**< Mobile network code */
    int lac;              			/**< Location area code */
    int ci;               			/**< Cell identity; as per 3G TS 25.331, 10.3.2.2 (28 bits) */
    int uraId;            			/**< URA identity */
    int psc_cellParameterId;          /**< Primary scrambling code for FDD or Cell parameter id for TDD */
    int arfcn;            			/**< Absolute radio frequency channel number */

    int t3212;            			/**< Periodic LA update timer (T3212) in minutes */
    int t3312;            			/**< Periodic RA update timer (T3312) in minutes */

    int hcsUsed;          		/**< Hierarchical cell structure used? \sa CCI API Ref Manual */
    int attDetAllowed;    		/**< Attach-detach allowed? \sa CCI API Ref Manual */


    int csDrxCycleLen;    		/**< CS domain DRX cycle length */
    int psDrxCycleLen;    		/**< PS domain DRX cycle length */
    int utranDrxCycleLen; 	/**< UTRAN DRX cycle length */
    int HSDPASupport;		/**< TRUE - serving cell supports HSDPA; FALSE - other. \sa CCI API Ref Manual */
    int HSUPASupport;		/**< TRUE - serving cell supports HSUPA; FALSE - other. \sa CCI API Ref Manual */

    int  lenOfMnc;             /**< Length of MNC, value range (2,3) */
} rilDevUmtsServingCellParameters;

typedef struct rilCipherAlgorithmInfo_s
{
    int algPresent;    	/**< Indicates if an algorithm is defined \sa CCI API Ref Manual */
    int cipherAlg;		/**< Cipher algorithm type \sa CiCipherAlgorithmType  */
    int cipherOn;      	/* Ciphering status = on/off \sa CCI API Ref Manual */
}  rilCipherAlgorithmInfo;

typedef struct rilDevUmtsUeOperationStatus_s
{
    int rrcState;         				/**< RRC state \sa CiDevEngModeUeRrcState */
    int numLinks;         				/**< Number of radio links */

    int srncId;           				/**<  U-RNTI: SRNC identifier */
    int sRnti;            					/**<  U-RNTI: S-RNTI */
    rilCipherAlgorithmInfo   				csCipherInfo;     				/**<  CS domain ciphering information. \sa CiCipherAlgorithm_struct */
    rilCipherAlgorithmInfo   				psCipherInfo;     				/**<  PS domain ciphering information. \sa CiCipherAlgorithm_struct */
    int HSDPAActive;				/**<  TRUE - HSDPA is currently activated; FALSE - other  \sa CCI API Ref Manual  */
    int HSUPAActive;				/**<  TRUE - HSUPA is currently activated; FALSE - other  \sa CCI API Ref Manual  */
    int MccLastRegisteredNetwork;	/**<  Mcc of last registered network */
    int MncLastRegisteredNetwork;	/**<  Mnc of last registered network */
    int TMSI;						/**<  TMSI */
    int PTMSI;						/**<  PTMSI */
    int IsSingleMmRejectCause;		/**<  TRUE - only one MM reject cause reported during the last engineering information period; FALSE - other \sa CCI API Ref Manual  */
    int IsSingleGmmRejectCause;	/**<  TRUE - only one GMM reject cause reported during the last engineering information period; FALSE - other \sa CCI API Ref Manual  */
    int	MMRejectCause;				/**<  The one that was reported during the last engineering information period reject cause (10.5.3.6) sent in CM Service Reject, Abort, MM-Status, and Location Updating Reject messages to MM from the network. \sa CiDevEngMMRejectCauseCodeType */
    int	GMMRejectCause;			/**<  Reported during the last engineering information period  \sa CiDevEngGMMRejectCauseCodeType */
    int mmState;					/**<  MM state refer to 3GPP 24.008 section 4.1.2  */  /* see enum MmState in Mm_comm.h. */
    int gmmState;					/**<  GMM state refer to 3GPP 24.008 section 4.1.3  */  /* see enum GmmState in Gmm_comm.h */
    int gprsReadyState;				/**<  0 - IDLE_STATE / 1 - STANDBY_STATE / 2 - READY_STATE. */ /* see enum GprsReadyState in grrmrtyp.h. */
    int readyTimerValueInSecs;		/**<  MM ready timer value in sec [value >0] */
    int NumActivePDPContext;		/**<  Number of active PDP contexts */
	int ULThroughput;           /**< UL throughput in octets per second */
	int DLThroughput;           /**< DL throughput in octets per second */
    int serviceStatus;              /**< Service status */ /* see enum ServiceStatus in Mmr_sig.h. */
    int pmmState;                   /**< UMM state for PS services */ /* see enum UmmState in Mm_comm.h. */
    int LAU_status;                 /**<Current update status of the UE */ /* see enum LocationUpdateStatus in Mmr_sig.h. */
    int LAU_count;                  /**<LAU attempt counter as held by UE; number of consecutive LAU failures in current LAU procedure attempt */
    int RAU_status;                 /**<RAU status of the UE */ /* see enum GprsUpdateStatus in Mmr_sig.h. */
    int RAU_count;                  /**<RAU attempt counter as held by UE; number of consecutive RAU failures in current RAU procedure attempt*/
} rilDevUmtsUeOperationStatus;

typedef struct rilDevUmtsServingCellInfo_s
{
    int mode;
    int sCellMeasPresent; 	/**< TRUE - is present; FALSE - is not present. \sa CCI API Ref Manual */
    int sCellParamPresent;	/**< TRUE - is present; FALSE - is not present. \sa CCI API Ref Manual */
    int ueOpStatusPresent;	/**< TRUE - is present; FALSE - is not present. \sa CCI API Ref Manual */
    int res1U8;				/**< padding */
    rilDevUmtsServingCellMeasurements  	sCellMeas;  			/**< SCell measurements \sa CiDevUmtsServingCellMeasurements_struct */
    rilDevUmtsServingCellParameters    	sCellParam; 			/**< PLMN/cell parameters \sa CiDevUmtsServingCellParameters_struct 	*/
    rilDevUmtsUeOperationStatus        	ueOpStatus; 			/**< UE operation status \sa CiDevUmtsUeOperationStatus_struct 		*/
} rilDevUmtsServingCellInfo;
 ...
 * ((const char **)data)[3] = new password verification
 *
 * "response" is NULL
 *
 * Valid errors:
...
/*SIMCom zhousijin 20200826 add for simcomdemo get_iccid bgin*/
#ifdef __SIMCOM_DEMO_SIM__
#define RIL_REQUEST_GET_ICCID 132
#endif
/*SIMCom zhousijin 20200826 add for simcomdemo get_iccid end*/


#ifdef __SIMCOM_DEMO_WDS__
/**
 * set apn method
 * SIMCOM WP add this rqst ID for openSDK set apn func
*/

#define RIL_REQUEST_OPENSDK_APN_HANDLER 133
#endif

#ifdef __SIMCOM_DEMO_NAS__
#define RIL_REQUEST_CREG_REGISTRATION_STATE 134
#endif
/***********************************************************************/
...
/**
 * RIL_UNSOL_RESPONSE_SMS_SENT
 *
 * Called when new SMS is sent.
 *
 * "data" is NULL
 */

#define RIL_UNSOL_RESPONSE_SMS_SENT 1045

#define RIL_UNSOL_RESPONSE_SMS_PP_DOWNLOAD 1046
...
typedef void (*usrTimeOutCallback)(void *param);

struct RIL_UBUS
{
    int (*RilUbusSend)(void *reqdata, int errno, void *data, unsigned int len, int reqid);
    int (*RilUbusNotify)(void *data, unsigned int len, int id);
    void (*RilUbusAddTimeout)(void *data, usrTimeOutCallback cb, const struct timeval *relativeTime);
};
/**
