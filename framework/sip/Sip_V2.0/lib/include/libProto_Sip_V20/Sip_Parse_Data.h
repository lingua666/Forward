...
namespace	_sip_v20_ {
	...
	{
	public:
		typedef Sip_Parse_Request			self_type;
	...
	public:
		...
		Sip_Parse_Request(const self_type& t1);

		~Sip_Parse_Request(void);

		Sip_Parse_Request& operator=(const Sip_Parse_Request& t1);
		...
		_string_t	FindData(sip_hdr_e Hdr_e,
			const _string_t& Start = "SOF",
			const _string_t& End = "EOF");

		//获取内容
		const _string_t& GetContent(void);
	};
