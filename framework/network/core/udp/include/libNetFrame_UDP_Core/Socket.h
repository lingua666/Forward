...
#ifndef __UDP_SOCKET_20150914105604_1442199364_H__
#define __UDP_SOCKET_20150914105604_1442199364_H__
...
namespace _io_net_{
	...
	namespace _udp_{
		...
		typedef _iocp_net_::_udp_::io_service		io_service;
		typedef _iocp_net_::_udp_::UdpSocket		socket_type;
		typedef _iocp_net_::_udp_::tagOverlapped	tagOverlapped;
	#elif defined( PLATFORM_OS_FAMILY_UNIX )
		typedef _epoll_net_::_udp_::io_service		io_service;
		typedef _epoll_net_::_udp_::UdpSocket		socket_type;
		typedef _epoll_net_::_udp_::tagOverlapped	tagOverlapped;
	#endif
		...
		public:
			...
			template <typename T1>
			int	AsyncSend( sockaddr_in* pAddr, const _string_type& sData, const T1& handle )
			{
				_WLock.Lock();
				if( isOpen() )
				{	
					...
					wBuf.buf = sData.c_str();
					wBuf.len = sData.size();
					return _Sock.async_write(pAddr, wBuf,handle);	
				}
				_WLock.UnLock();
				return -1;
			}
			...
			template <typename T1>
			int	AsyncSend( sockaddr_in* pAddr, const char* c_pData, UInt32 u32Size, const T1& handle )
			{
				_WLock.Lock();
				if( isOpen() )
				{
