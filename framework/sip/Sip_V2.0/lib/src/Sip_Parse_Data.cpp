...
#include <libProto_Sip_V20/Sip_Parse_Data.h>
...
namespace	_sip_v20_ {
	...
	Via_Hdr_INFO Sip_IParse::GetFirstVia(void)
	{
	...
	Sip_Parse_Request::Sip_Parse_Request(const self_type& t1)
	{
		this->_Common = t1._Common;
		this->_sMethod = t1._sMethod;
		for (Msg_Map::iterator iter = t1._Map.begin(); iter != t1._Map.end(); iter ++)
		{
			_Map.insert(std::make_pair(iter->first, iter->second));
		}
	}
	...
	Sip_Parse_Request& Sip_Parse_Request::operator=(const Sip_Parse_Request& t1)
	{
		this->_Common = t1._Common;
		this->_sMethod = t1._sMethod;
		for (Msg_Map::iterator iter = t1._Map.begin(); iter != t1._Map.end(); iter++)
	...
	//获取内容
	const _string_t& Sip_Parse_Request::GetContent(void)
	{
		Msg_Map::iterator iter = _Map.find(sip_hdr_names[SIP_H_CONTENT_LENGTH]);
		if (iter != _Map.end())
		{
	...
	int Sip_Parse_Response::ParseToMap(const char* c_szBuf, UInt32 uSize, Msg_Map* pMap)
	{
		...
		{
			//test
			printf("Sip_Parse_Response::ParseToMap() Failed\r\n");
			//test
		}
	...
	//获取内容
	const _string_t& Sip_Parse_Response::GetContent(void)
	{
		Msg_Map::iterator iter = _Map.find(sip_hdr_names[SIP_H_CONTENT_LENGTH]);
		if (iter != _Map.end())
		{
