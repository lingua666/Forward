...
public:
	...
	{
		_px = malloc( sizeof(sp_counted_impl_p) + uSize + 1 );
		return new (_px) sp_counted_impl_p( _px + sizeof(sp_counted_impl_p) );
	}
...
public:
	...
	{
		return new(pBuf) sp_counted_impl_buf( pBuf + _OBJ_SIZE );
	}
...
private:
	...
	_Elem			_Buf[_BUF_SIZE + _OBJ_SIZE + 1];
...
public:
		...
		, _Myres(t1._Myres)
	{
		if( _Myres > _BUF_SIZE )
			_Alval = t1._Alval;
		else if( t1._Mysize > 0 )
			std::memcpy(_Buf, t1._Buf, _BUF_SIZE);
	}
	...
	string& operator=( const string& _Right )
	{
		if( !_Inside(_Right._Myptr()) )
		{
			...
			if( _Right._Myres > _BUF_SIZE )
				_Alval = _Right._Alval;
			else
			{
				_Alval.reset();
				memcpy(_Buf, _Right._Buf, _BUF_SIZE);
			}
		}
	...
	inline string& operator=( const _Elem* _Ptr )
	{	
		/*_Mysize = strlen(_Ptr);
		if(_Mysize > _Myres || _Alval.use_count() > 1)
		{
			assign(_Ptr, _Mysize, true);
			return *this;
		}

		if( _Myres > _BUF_SIZE )
		{
			std::memcpy(_Alval.get(), _Ptr, _Mysize);
			_Alval.get()[_Mysize] = 0;
		}
		else
		{
			std::memcpy(_Buf, _Ptr, _Mysize);
			_Buf[_Mysize] = 0;
		}*/
		...
		if(_Alval.use_count() > 1 || _Mysize > _Myres)
		{
			assign(_Ptr, _Mysize, true);
			return *this;
		}

		std::memcpy(_Myptr(), _Ptr, _Mysize);
		_Myptr()[_Mysize] = 0;
		return *this;
	}
	...
	{	
		assign(_Right._Myptr(), _Right.size());
		return *this;
	}
	...
	_Elem *_Myptr()
	{
		return _Alval ? _Alval.get() : (char*)_Buf;
	}
	...
	{	
		...
		if (this->_Myres < _Newsize)
		{
			...
			{
				char old[_BUF_SIZE];
				memcpy(old, _Buf, _BUF_SIZE);
				_Grow(_Newsize, true);

				if( _Off > 0 )
					std::memcpy(_Myptr(), old, _Off);	// fill hole

				std::memcpy(_Myptr() + _Off, _Right + _Roff, _Count);	// fill hole

				if( _Nm > 0 )
					std::memcpy(_Myptr() + _Off + _Count, old + _Off + _N0, _Nm);	// fill hole
			}
		...
		else
		{
			...
			if( _Nm > 0 )
				std::memmove(_Myptr() + _Off + _Count, _Myptr() + _Off + _N0, _Nm);	// fill hole
	...
	{	// return [_Off, _Off + _Count) as new string
		return (string(&(_Myptr()[_Off]), _Count));
	}
...
{	
	...
	while(true)
	{
		...
		{
			string s;
			string s1("12345678923456789456789");
			_timestamp_::Timestamp	time;
			for(int i = 0; i < num; i ++)
			{
				s = s1;
		...
		{
			std::string s;
			std::string s1("12345678923456789456789");
			_timestamp_::Timestamp	time;
			for(int i = 0; i < num; i ++)
			{
				s = s1;
		...
		{
			{
				...
				for(int i = 0; i < num; i ++)
				{
					s = string("12346567989798798564654632132111111111111111111111111");
				}
...

			{
				...
				for(int i = 0; i < num; i ++)
				{
					s = std::string("12346567989798798564654632132111111111111111111111111");
				}
		...
		{
			string s("1234656798979879856465463213211213121");
			_timestamp_::Timestamp	time;
			for(int i = 0; i < num; i ++)
			{
		...
		{
			std::string s("1234656798979879856465463213211213121");
			_timestamp_::Timestamp	time;
			for(int i = 0; i < num; i ++)
			{
				s = "123465679897987985646546321321";
...

		{
			...
			for(int i = 0; i < num; i ++)
			{
				string s = ss.substr(10, 30);
			}
			printf("string23 time:%d\r\n", (_timestamp_::Timestamp().epochMicroseconds() - time.epochMicroseconds()) / 1000);
		}
