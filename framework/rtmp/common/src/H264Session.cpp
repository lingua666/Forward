...
namespace _rtmp_ {
	...
	int H264Session::Init(const HFNEvent& hEvent)
	{
		_pVideo = new (std::nothrow) H264Info();
		if (_pVideo == NULL)
		{
			LOG_Print_Info(libFrameWork_RtmpCommon, "H264Session::Init() new H264Info Failed!!");
			return -1;
		}
	...
	void H264Session::Release(void)
	{
		Close();
	}

	int H264Session::Connect(const _rtmp_string& sUrl)
	{
		RTMP_Init(&_Rtmp);
		...
		{
			LOG_Print_Error(libFrameWork_RtmpCommon, "H264Session::Connect() SetupURL:%s Failed!");
			return -1;
		}
		...
		{
			LOG_Print_Error(libFrameWork_RtmpCommon, "H264Session::Connect() ConnectImpl:%s Failed!");
			return -1;
		}
	...
	int H264Session::Close(bool isPassive)
	{
		if (_isOpen)
		{
			_isOpen = false;
	...
	int H264Session::ConnectImpl(RTMP *r, RTMPPacket *cp, const _rtmp_string& sUrl)
	{
		...
		if (GetAsynStreamClientsInstance()->Connect(sHost.c_str(), r->Link.port,
			function20_bind(&H264Session::HandleConnect, this, shared_from_this(),
				sUrl, sHost, r->Link.port, _function_::_1)) == -1)
		{
			LOG_Print_Error(libFrameWork_RtmpCommon, "H264Session::ConnectImpl(%s:%d) Url:%s",
	...
	void H264Session::HandleConnect(const self_sptr& sptr, const _rtmp_string& sUrl,
		const _rtmp_string& sIP, UInt16 uPort, NETHANDLE hNet)
	{
		if (hNet == -1)
		{//连接失败
			LOG_Print_Error(libFrameWork_RtmpCommon, "H264Session::HandleConnect(%s:%d) Url:%s Failed!!",
		...
		else
		{
			GetAsynStreamClientsInstance()->SetRecvHandle(hNet,
				function20_bind(&H264Session::HandleRecv, this, shared_from_this(),
					_function_::_1, _function_::_2, _function_::_3));

			if (GetAsynStreamClientsInstance()->SetDestroyHandle(hNet,
				function20_bind(&H264Session::HandleDestroy, this, shared_from_this(), _function_::_1)) == -1)
			{//失败
				LOG_Print_Error(libFrameWork_RtmpCommon, "H264Session::HandleConnect(%s:%d) SetDestroyHandle Url:%s Failed!!",
					sIP.c_str(), uPort, sUrl.c_str());
		...
		{
			LOG_Print_Error(libFrameWork_RtmpCommon, "H264Session::HandleConnect(%s:%d) CreateNetConnect Url:%s Failed!!",
				sIP.c_str(), uPort, sUrl.c_str());
	...
	int H264Session::Send(RTMPSockBuf *sb, const char *buf, int len)
	{
		int rc;
	...
	int H264Session::SendPacket(const char* c_szH264Data, UInt32 uH264Size)
	{
		if (!IsOpen())
			return -1;
		...
		PacketList Packets;
		if (H264Session::ParseH264Data(c_szH264Data, uH264Size, Packets) == 1)
		{
	...
	int H264Session::ParseH264Data(const char* c_szInData, UInt32 uInSize, PacketList& Packets)
	{
		...
		if (iStartPos >= 0)
		{
			...
			if (PlayloadInfo.isFull)
			{
				if (_pVideo->isReady() && PlayloadInfo.uHeadType != NAL_UNIT_TYPE_SEI)
				{
					...
					{
						_rtmp_::H264Session::tagVideoInfo	VideoInfo;
...
gt_end:
	...
	void H264Session::HandleRecv(const self_sptr& sptr, NETHANDLE hNet, const StreamBuf_ptr& Buf_ptr, long lRemain)
	{
		//test
...
gt_start:
		...
		case RTMP_STATUS_HandShake_S0S1S2:
		{
			...
			{//发送响应
				...
				LOG_Print_Info(libFrameWork_RtmpCommon, "H264Session::HandleRecv() %s: Type Answer   : %02X", __FUNCTION__, type);

				if (type != 0x03)
					LOG_Print_Info(libFrameWork_RtmpCommon, "H264Session::HandleRecv() %s: Type mismatch: client sent %d, server answered %d",
				...
				LOG_Print_Info(libFrameWork_RtmpCommon, "H264Session::HandleRecv() %s: Server Uptime : %d", __FUNCTION__, suptime);
				LOG_Print_Info(libFrameWork_RtmpCommon, "H264Session::HandleRecv() %s: FMS Version   : %d.%d.%d.%d", __FUNCTION__,
					serversig[4], serversig[5], serversig[6], serversig[7]);

				/*bMatch = (memcmp(&_sData[1 + RTMP_SIG_SIZE], clientsig, RTMP_SIG_SIZE) == 0);
				if (!bMatch)
				{
					LOG_Print_Error(libFrameWork_RtmpCommon, "H264Session::HandleRecv() %s, client signature does not match!", __FUNCTION__);
					Close();
					return ;
				}*/
				...
				{
					LOG_Print_Error(libFrameWork_RtmpCommon, "H264Session::HandleRecv() %s, RTMP connect failed.", __FUNCTION__);
					Close();
		...
		case RTMP_STATUS_CONNECT_STREAM:
		{
				...
				ReadPacketV20(&_Rtmp, &_RTMPPacket, _sData.c_str(), _sData.size(), OutSize))
			{
				if (RTMPPacket_IsReady(&_RTMPPacket))
				{
					...
					{
						LOG_Print_Info(libFrameWork_RtmpCommon, "H264Session::HandleRecv() Received FLV packet before play()! Ignoring.");
	...
	void H264Session::HandleDestroy(const self_sptr& sptr, NETHANDLE hNet)
	{
		if (_isOpen)
		{
			Close(true);
	...
	int H264Session::SendSpsPps(void)
	{
		RTMPPacket_sptr Packet_sptr = AllocPacket();
		RTMPPacket* pPacket = Packet_sptr.get();
