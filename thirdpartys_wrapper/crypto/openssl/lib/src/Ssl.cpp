...
namespace	_openssl_ {
	...
	int Ssl::Encode(const char* c_szData, UInt32 uSize)
	{
		...
		LOG_Print_Info(libThirdWrap_Openssl, "Ssl::Encode(%d) Data:%s", uSize, c_szData);
		...
		if (w <= 0)
		{
			...
			{// 检测到对方安全关闭 SSL 连接
				LOG_Print_Error(libThirdWrap_Openssl, "Ssl::Encode() The Other Side is Closed error:%d, Data(%d):%s",
					err, uSize, c_szData);
			...
			{	//本地错误
				LOG_Print_Error(libThirdWrap_Openssl, "Ssl::Encode() Failed, error:%d, Data(%d):%s",
					err, uSize, c_szData);
	...
	Ssl::_ssl_string	Ssl::Decode(const char* c_szData, UInt32 uSize)
	{
		...
		if (r <= 0)
		{
			...
			{// 检测到对方安全关闭 SSL 连接
				LOG_Print_Error(libThirdWrap_Openssl, "Ssl::Decode() The Other Side is Closed error:%d!!", err);
			...
			{// 本地错误
				LOG_Print_Error(libThirdWrap_Openssl, "Ssl::Decode() Failed, error:%d!!", err);
