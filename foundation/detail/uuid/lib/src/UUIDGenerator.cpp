...
#include <libUUID/DigestEngine.h>
...
UUID UUIDGenerator::create()
{
	...
	Timestamp_type::UtcTimeVal tv = timeStamp();
	_Lock.UnLock();
...
UUID UUIDGenerator::createFromName(const UUID& nsid, const _string_type& name)
{
	MD5Engine md5;
...
UUID UUIDGenerator::createFromName(const UUID& nsid, const _string_type& name, DigestEngine& de)
{
...
Timestamp_type::UtcTimeVal UUIDGenerator::timeStamp()
{
	Timestamp_type now;
	for (;;)
	{
	...
	Timestamp_type::UtcTimeVal tv = now.utcTime();
	return tv + _ticks;
}
...
{
	static UUIDGenerator	s_UUIDGenerator;
	return &s_UUIDGenerator;
}


_string_type UUIDGenerator::UUIDToString( void )
{
	 return UUIDGenerator::defaultGenerator()->createRandom().toString();
}

_string_type	UUIDGenerator::UUIDHexToString( void )
{
	return UUIDToString().replace("-", "");
}
