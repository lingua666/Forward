...
#include <libNetFrame_AsyncFileClients/FileClients.h>

_client_::_async_::FileClients		Clients;
typedef _tcp_session_::StreamBuf		StreamBuf;
typedef _tcp_session_::StreamBuf_ptr	StreamBuf_ptr;
...
void ClientClose( _string_type sIP, UInt16 uPort, NETHANDLE Node )
{
	CASSubAndFetch(&g_iNumber);
	printf("ClientClose(%d):%s:%d\r\n",g_iNumber,sIP.c_str(), uPort);
}
