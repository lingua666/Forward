...
namespace	_validity_{
	...
	int DllFile::GetKeySize( void )
	{
		return KEY_SIZE_20160713_DEF + KEY_RESERVE_20160713_DEF;
	}

	const char* DllFile::GetHead( void )
	{
		static char s_Key20160713[KEY_RESERVE_20160713_DEF] = 
							{0xEE, 0xFF, 0xE2, 0x82, 0xAC, 0x03, 0x0, 0x04, 0x0};
		return s_Key20160713;
	}

	int DllFile::GetKey( const char* c_szPath, const char* SubKey,
	...
	int DllFile::ReadKey( _file_::CMyFile& hFile, char* szBuf, UInt16 uSize )
	{
...
gt_end:
	...
	int DllFile::Encode( _file_::CMyFile& hFile,
					const char* SubKey,
					const char* pOut,
					UInt16 uOutSize )
	{
	...
	int DllFile::ChangePostion( const char* c_szInput, UInt16 uInSize,
	...
	bool DllFile::isExsitKey( const char* c_szPath )
	{
		_file_::CMyFile	hFile;
		if( hFile.Open(c_szPath, "rb") != 1 )
			return false;
