...
typedef _string_v30_::string< MemPool_macro<200> >	string_type;
...
int main(int argc, char* argv[])
{
	while(true)
	{
		...
		{
			{
				string_type s = "123465679897987985";
				int iRad = rand() % 200;
				string_type s23;
				for (int j = 0; j < iRad; j ++)
				{
					s23 += string_type::NumberToStr(j);
				}
				s23 = s;
			}
	...
	do 
	{
		...
		if( Pos != -1 )
		{
			ss = sssss.substr(Old, Pos - Old);
			Pos ++;
			Old = Pos;
		}
		else if(Old < sssss.size())
		{
			ss = &sssss[Old];
		}
		printf("ss:%s\r\n", ss.c_str());
	...
	string_type::NumberToStr(10000);// == 100

	string_type stype = "车道指示器";
	string_type sey = stype.gbk_utf8();
	const char* ps = sey.c_str();
	sey.append(1,'<');

	string_type sey1 = sey.utf8_gbk();
...

	{
		...
		{
			s == "10000";
			//string_type::StringToInt("10");
		}
		printf("string_type00 time:%lld\r\n", (_timestamp_::Timestamp().epochMicroseconds() - time.epochMicroseconds()) / 1000);
	}

	{
		...
		for(int i = 0; i < num; i ++)
		{
			string_type::StringToInt(s);// == 100;
		}
		printf("string_type11 time:%lld\r\n", (_timestamp_::Timestamp().epochMicroseconds() - time.epochMicroseconds()) / 1000);
	}
