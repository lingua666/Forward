...
public:
	static sp_counted_impl_p* GetInstance( UInt32 uSize )
	{
		void* pBuf = malloc( sizeof(sp_counted_impl_p) + uSize + 1 );
		return new (pBuf) sp_counted_impl_p( reinterpret_cast<char*>(pBuf) );
	}

public:
	sp_counted_impl_p( char* px ): _px( px ), _use_count(1)
	{
	...
	char*	get( void ) const
	{
		return &_px[sizeof(sp_counted_impl_p)];
	}
};
...
public:
	...
	explicit shared_count( UInt32 uSize ): _pi( 0 )
	{
