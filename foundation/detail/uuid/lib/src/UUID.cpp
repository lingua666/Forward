...
UUID::UUID(const _string_type& uuid)
{
	parse(uuid);
}
...
{
	//poco_check_ptr (uuid);
	EXCEPTION_THROW(CMyException,"Point is NULL!")
	parse(_string_type(uuid));
}
...
void UUID::parse(const _string_type& uuid)
{
	if (uuid.size() < 36)
		EXCEPTION_THROW(CMyException,uuid.c_str())

	if (uuid[8] != '-'|| uuid[13] != '-' || uuid[18] != '-' || uuid[23] != '-')
		EXCEPTION_THROW(CMyException,uuid.c_str())
	
	_string_type::const_iterator it = uuid.begin();
	_timeLow = 0;
	for (int i = 0; i < 8; ++i)
	{
...
_string_type UUID::toString() const
{
	_string_type result;
	result.reserve(36);
...
void UUID::appendHex(_string_type& str, UInt8 n) 
{
...
void UUID::appendHex(_string_type& str, UInt16 n)
{
...
void UUID::appendHex(_string_type& str, UInt32 n)
{
