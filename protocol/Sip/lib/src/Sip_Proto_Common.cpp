...
namespace	_sip_ {
	...
	const Via_Hdr_INFO& Sip_Proto_Common::GetFirstVia(void)
	{
	...
	void Sip_Proto_Common::GenerateToList(Hdr_List* pList)
	{
		...
		sptr->Content += "<sip:" + _From.Coding;
		sptr->Content += "@" + _From.Domain + ">";
		if (_From.Tag.size() > 0)
		{
		...
		sptr->Content += "<sip:" + _To.Coding;
		sptr->Content += "@" + _To.Domain + ">";
		if (_To.Tag.size() > 0)
		{
	...
	bool Sip_Proto_Common::ParseHdr(const Hdr_INFO_sptr& sptr)
	{
		...
		if (sptr)
		{
			...
			{//From
				_string_t Coding = Sip_Parse::FindData(sptr->Content, "<sip:", "@");
				_string_t Domain = Sip_Parse::FindData(sptr->Content, "@", ">");
