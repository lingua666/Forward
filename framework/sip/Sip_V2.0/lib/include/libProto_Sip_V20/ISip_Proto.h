...
#ifndef __SIP_V20_ISIP_PROTO_20160510104022_1462848022_H__
#define __SIP_V20_ISIP_PROTO_20160510104022_1462848022_H__
...
namespace	_sip_v20_ {
	...
	*/
	...
	struct Host_Port_INFO
	{
		...
		Host_Port_INFO(const Host_Port_INFO& t1)
		{
			this->Host = t1.Host;
			this->Port = t1.Port;
		}

		Host_Port_INFO& operator=(const Host_Port_INFO& t1)
		{
			this->Host = t1.Host;
			this->Port = t1.Port;
			return *this;
	...
	struct UriDetails_INFO
	{
		_string_t	Coding;
		Host_Port_INFO	Host;

		UriDetails_INFO(void)
		{

		}

		UriDetails_INFO(const UriDetails_INFO& t1)
		{
			this->Coding = t1.Coding;
			this->Host.Host = t1.Host.Host;
			this->Host.Port = t1.Host.Port;
		}

		UriDetails_INFO& operator=(const UriDetails_INFO& t1)
		{
			this->Coding = t1.Coding;
			this->Host.Host = t1.Host.Host;
			this->Host.Port = t1.Host.Port;
	...
	struct Via_Hdr_INFO
	{
		...
		Via_Hdr_INFO(const Via_Hdr_INFO& t1)
		{
			this->isUdp = t1.isUdp;
			this->Sent_by.Host = t1.Sent_by.Host;
			this->Sent_by.Port = t1.Sent_by.Port;
			this->Ttl = t1.Ttl;
			this->Rport = t1.Rport;
			this->Maddr = t1.Maddr;
			this->Recvd = t1.Recvd;
			this->Branch = t1.Branch;
		}

		Via_Hdr_INFO& operator=(const Via_Hdr_INFO& t1)
		{
			this->isUdp = t1.isUdp;
			this->Sent_by.Host = t1.Sent_by.Host;
			this->Sent_by.Port = t1.Sent_by.Port;
	...
	*/
	...
	//protected:
		...
		static _string_t  GenerateTime(void);

		static UriDetails_INFO ConvertFromUri(const _string_t& sUri);

		static bool IsIP(const _string_t& Content);
